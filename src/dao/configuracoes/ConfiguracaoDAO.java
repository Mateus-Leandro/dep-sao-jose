	...
	public Boolean salva_configuracao(Configuracoes configuracao) {
		...
		try {
			...
			ps = conn.prepareStatement("INSERT INTO configuracoes "
					+ "(nome_empresa, responsavel, cnpj, tel_fixo, celular, email, endereco, salva_parc_dif, alt_orc, gera_pdf, vincula_barras, idConsumidor_final) "
					+ "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			ps.setString(1, configuracao.getNome_empresa());
			ps.setString(2, configuracao.getResponsavel());
			ps.setString(3, configuracao.getCNPJ());
			ps.setString(4, configuracao.getTel_fixo());
			ps.setString(5, configuracao.getCelular());
			ps.setString(6, configuracao.getEmail());
			ps.setString(7, configuracao.getEndereco());
			ps.setString(8, configuracao.getSalva_parc_dif());
			ps.setString(9, configuracao.getAltera_orc());
			ps.setString(10, configuracao.getGera_PDF());
			ps.setString(11, configuracao.getVincula_barras());
			ps.setInt(12, configuracao.getConsumidor_final().getId());
	...
	public Configuracoes busca_configuracoes() {
		...
		try {
			ps = conn.prepareStatement(
					"SELECT nome_empresa, responsavel, cnpj, tel_fixo, configuracoes.celular, "
